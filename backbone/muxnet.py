...
from backbone.conv2d_helpers import select_conv2d, SelectAdaptivePool2d
...
class GenMUXNet(nn.Module):
    ...
    def __init__(self, block_args, num_classes=1000, in_chans=3, stem_size=32, num_features=1280,
        ...
        #self.plant_classifier = nn.Linear(self.num_features * self.global_pool.feat_mult(), 14)
        #self.disease_classifier = nn.Linear(self.num_features * self.global_pool.feat_mult(), 21)
        #self.classifier = nn.Linear(self.num_features * self.global_pool.feat_mult(), self.num_classes)
        self.output_layer = nn.Sequential(nn.BatchNorm2d(self.num_features),
                                          nn.Dropout(self.drop_rate),
                                          nn.Flatten(),
                                          nn.Linear(self.num_features * 4 * 4, self.num_classes),
    ...
    def forward(self, x):
        x = self.forward_features(x, pool=False)
        #if self.drop_rate > 0.:
        #    x = F.dropout(x, p=self.drop_rate, training=self.training)
        return self.output_layer(x)
        #return x
        #return self.plant_classifier(x), self.disease_classifier(x)
...
def _gen_muxnet_l(channel_multiplier=1.0, depth_multiplier=1.0, num_classes=1000, divide=False, **kwargs):
...
#@register_model
def muxnet_l(pretrained=False, num_classes=1000, in_chans=3, divide=False, **kwargs):
    """ MUXNet-l """
    default_cfg = default_cfgs['muxnet_m']
    # NOTE for train, drop_rate should be 0.2
    ...
    model = _gen_muxnet_l(
        channel_multiplier=1.0, depth_multiplier=1.0,
        num_classes=num_classes, in_chans=in_chans, divide=divide, **kwargs)
    model.default_cfg = default_cfg
    if pretrained:
...
def factory(name, **kwargs):
    if name == 'muxnet_m':
        ...
        return muxnet_l(divide=True, **kwargs)        
    else:
